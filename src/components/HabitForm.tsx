  ...
  return (
    ...
    >
      <section>
        ...
        <input
          className="mt-3 w-full rounded-3xl border-0 bg-[#f5f5f7] px-6 py-5 text-2xl font-bold tracking-tight outline-none"
      ...
      <section className="grid grid-cols-3 gap-2 rounded-3xl bg-[#f5f5f7] p-2">
        {(['daily', 'weekly_specific', 'monthly_specific'] as Habit['frequency'][]).map((option) => (
          <button
            key={option}
            type="button"
            onClick={() => setFrequency(option)}
            className={`tap-active rounded-2xl px-3 py-2 text-xs font-bold uppercase tracking-[0.22em] ${frequency === option ? 'bg-white text-black shadow-sm' : 'text-[#888]'}`}
          >
        ...
        <button
          ...
          className={`tap-active col-span-2 rounded-2xl px-3 py-2 text-xs font-bold uppercase tracking-[0.22em] ${frequency === 'flexible' ? 'bg-white text-black shadow-sm' : 'text-[#888]'}`}
        >
          Flexible Count
        </button>
        <button
          ...
          className={`tap-active rounded-2xl px-3 py-2 text-xs font-bold uppercase tracking-[0.22em] ${frequency === 'once' ? 'bg-white text-black shadow-sm' : 'text-[#888]'}`}
        >
          One-off
        </button>
      </section>
      ...
      {frequency === 'flexible' && (
        <section className="space-y-3">
          ...
          <div className="flex gap-3">
            ...
            <input type="number" min={1} value={targetIntervalCount} onChange={(e) => setTargetIntervalCount(Number(e.target.value))} className="w-full rounded-2xl border-0 bg-[#f5f5f7] px-4 py-3 text-xl font-bold" />
          </div>
        </section>
      )}
      ...
      <section className="rounded-3xl bg-[#f7f7f8] p-4">
        <div className="grid grid-cols-5 gap-3">
          {icons.map((item) => (
            <button key={item} type="button" className={`tap-active rounded-2xl bg-white/70 py-3 text-2xl ${icon === item && !customIcon ? 'ring-2 ring-black' : ''}`} onClick={() => { setIcon(item); setCustomIcon(''); }}>
              {item}
            </button>
          ))}
        </div>
        <input className="mt-3 w-full rounded-2xl border-0 bg-white/80 px-5 py-4 text-center text-3xl" placeholder="Or type an Emoji..." value={customIcon} onChange={(e) => setCustomIcon(e.target.value.slice(0, 2))} />
      </section>

      <section className="grid grid-cols-2 gap-5">
        <div>
          <p className="micro-label">Daily Goal</p>
          <div className="mt-3 rounded-3xl bg-[#f5f5f7] px-6 py-6 text-4xl font-black">{goal}<span className="ml-3 text-lg text-[#c3c3c7]">{frequencyLabel}</span></div>
      ...
      <section>
        ...
        <input className="mt-3 w-full rounded-3xl border-0 bg-[#f5f5f7] px-6 py-5 text-4 tracking-tight text-lg" placeholder="https://... or anki://..." value={externalUrl} onChange={(e) => setExternalUrl(e.target.value)} />
      </section>
      ...
      <button type="submit" className="tap-active w-full rounded-full bg-black py-5 text-[13px] font-black uppercase tracking-[0.6em] text-white">
        Confirm
      </button>
    </form>
  );
