...
export const isHabitDue = (habit: Habit, dateKey: string): boolean => {
...
export const calculateStats = (habits: Habit[], entries: Entry[]) => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    habits.forEach(h => {
      ...
      if (h.frequency !== 'once' && isHabitDue(h, k)) {
        dueCount++;
  ...
  for (let i = 0; i < 365; i++) {
    ...
    const dueHabits = habits.filter(h => h.frequency !== 'once' && isHabitDue(h, k));
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const dueHabits = habits.filter(h => isHabitDue(h, k));
    let val = 0;
