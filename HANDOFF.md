...
## 10. 最新更新（2026-02-17: P0/P2 UX刷新 + リファクタリング）

### 実施内容（P0）
1. **Today画面を3層構造へ再編**
   - 上段: 進捗サマリーカード（完了/対象、未完了数、完了数）
   - 中段: 「今やるタスク」セクション（優先表示: 単発→柔軟→通常）
   - 下段: 完了済みタスクの折りたたみ表示（継続）
2. **Habit作成フォームをステップ化**
   - 4ステップ（基本 / 頻度 / 目標 / 詳細）
   - 必須入力（習慣名）を満たしてから次へ進める
   - 目標回数にプリセット（1/3/5/10）を追加
3. **下部ナビの情報設計を強化**
   - 記号アイコン中心→意味ラベル中心（英語+日本語）
   - アクティブ状態に `aria-current` / 下線 / 背景差分を追加

### 実施内容（P2）
1. **文言トーンの統一（日本語寄せ）**
   - Today/Analysis/Habits/Settings/Login の見出し・主要導線文言を統一
2. **アクセシビリティ改善**
   - フォーカスリング (`:focus-visible`) をグローバルで付与
   - スクロールバーを非表示から可視へ変更（WebKit/Firefox両対応）
   - micro-label のコントラストを改善

### リファクタリング
- Today画面の算出ロジックを `useMemo` で明示（due/active/completed）し、優先度算出関数を分離。
- HabitForm はステップ制御と送信処理を明確分離し、責務を整理。

### 実行済み確認
- `npm test` : PASS（8 files / 51 tests）
- `npm run build` : PASS

### 追加メモ
- UI変更のスクリーンショット取得済み（`/login`）。
